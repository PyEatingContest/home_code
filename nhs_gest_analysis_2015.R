...
original <- read_sas('./data sets/natl2015.sas7bdat')
skinny <- original %>% 
    select(-matches('.*race.*|.*cigs.*|.*hisp.*|.*RF_.*|.*facil.*'))
skinny %>% 
    mutate(gest_range = as.factor(case_when(
    skinny$gestrec10 == 1 ~ "Under 20 weeks",
        skinny$gestrec10 == 2 ~ "20 to 27 weeks",
        skinny$gestrec10 == 3 ~ "28 to 31 weeks",
        skinny$gestrec10 == 4 ~ "32 to 33 weeks",
        skinny$gestrec10 == 5 ~ "34 to 36 weeks",
        skinny$gestrec10 == 6 ~ "37 to 38 weeks",
        skinny$gestrec10 == 7 ~ "39 weeks",
        skinny$gestrec10 == 8 ~ "40 weeks",
        skinny$gestrec10 == 9 ~ "41 weeks",
        skinny$gestrec10 == 10 ~ "42 weeks and over",
        skinny$gestrec10 == 99 ~ "Unknown"))
    )

skinny_load <- readRDS("2015_Birth_Data_Skinny.rds")
tmp <- skinny_load %>% 
    select(dbwt, mager, f_tobaco, bmi, meduc, m_ht_in, wtgain, f_meduc, no_risks)
round(cor(tmp, use='complete.obs'),2)

mod_tmp <-
    lm(dbwt ~ mager + f_tobaco + meduc + f_meduc + no_risks + wtgain, data = tmp)
    
corrplot(cor(tmp, use='complete.obs'), type = "upper")


# Birthweight histogram
skinny_load %>%
    select(dbwt) %>%
    mutate(lbs = round(conv_unit(dbwt, "g", "lbs"),1)) %>% 
    filter(lbs <= 13) %>%   
    ggplot(aes(x = lbs)) +
    geom_histogram() + 
    coord_flip()
...
skinny %>% 
    select(dbwt, dmeth_rec) %>%
    mutate(lbs = round(conv_unit(dbwt, 'g', 'lbs'),2)) %>% 
    filter(lbs <= 13) %>% 
    group_by(dmeth_rec) %>% 
    summarize(avg = mean(lbs), med = median(lbs), births = n()) %>% 
    mutate(perc_births = round(births / sum(births) * 100, 2))

skinny %>% filter(gestrec10 != 99) %>% 
    ggplot(aes(x = gestrec10)) + 
    geom_bar() + 
    coord_cartesian(xlim = c(20,45))

saveRDS(skinny, "2015_Birth_Data_Skinny.rds")

### Everything below refers to 2005 data file ###
#sample <- original %>% sample_n(100)
mass <- original %>% 
    filter(statenat == 22)
